...
import { generateCampaign, generateSocialPosts, generateImage } from '../services/geminiService';
...
import type { WorkflowResult, GeneratedImageWithPost, Campaign, SocialPost, GeneratedImage } from '../types/index';
...
const WorkflowsScreen: React.FC = () => {
    ...
    const { setWorkflowResult, setActiveTool } = useMarketingTools();
    ...
    const runWorkflow = async () => {
        ...
        try {
            ...
            // Step 3: Generate Images for Posts
            ...
            const imagePromises = socialPosts.map(post => {
                ...
                return generateImage(imagePrompt, brandPersona).then(imgResult => ({
                    ...imgResult,
                    forPostContent: post.content,
                }));
            });
            const imageResults = await Promise.all(imagePromises);
            
            const images: GeneratedImageWithPost[] = imageResults.map(res => ({
                image: `data:image/png;base64,${res.imageBase64}`,
                prompt: res.forPostContent, // Simplified prompt for history
                forPostContent: res.forPostContent,
            }));

            // Finalize
            const result: WorkflowResult = { campaign, socialPosts, images };
    ...
    return (
        <div className="animate-fade-in max-w-2xl mx-auto h-full flex flex-col">
            <ToolHeader title={t.workflowsScreenTitle} onBack={() => setActiveTool(null)} />
                 ...
                 <div className="bg-white/60 dark:bg-gray-800/40 backdrop-blur-md border border-gray-200/80 dark:border-white/10 rounded-xl p-6 mb-8">
                    <div className="flex items-start gap-4">
                        <div className="flex-shrink-0 p-3 bg-indigo-500/10 dark:bg-indigo-500/20 rounded-lg">
                            <WorkflowIcon className="w-6 h-6 text-indigo-500 dark:text-indigo-400" />
                        </div>
                        <div>
