...
import type { Screen, NewProductLaunchWorkflowResult } from '../types/index';

interface NewProductLaunchWorkflowResultsScreenProps {
    setActiveScreen: (screen: Screen) => void;
}

const NewProductLaunchWorkflowResultsScreen: React.FC<NewProductLaunchWorkflowResultsScreenProps> = ({ setActiveScreen }) => {
    const { workflowResult, setWorkflowResult } = useMarketingTools();
    const { addPlannerItem } = usePlanner();
    const { t } = useTranslations();
    const [isAdded, setIsAdded] = useState(false);

    if (!workflowResult || !('campaign' in workflowResult.result)) return null;

    const { result } = workflowResult;
    const { campaign, socialPosts, images } = result as NewProductLaunchWorkflowResult;
...
export default NewProductLaunchWorkflowResultsScreen;